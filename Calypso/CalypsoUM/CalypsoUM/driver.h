...
namespace driver
{
	...
	template <typename T>
	T read(const uintptr_t address, PNTSTATUS out_status = 0)
	{
		T buffer{ };
		NTSTATUS status = read_memory(shared::game::pid, address, uintptr_t(&buffer), sizeof(T));
		if (out_status)
			*out_status = status;
		return buffer;
	}
	...
	template <typename T>
	T read_raw(const DWORD pid, const uintptr_t address, PNTSTATUS out_status = 0)
	{
		T buffer{ };
		NTSTATUS status = read_memory(pid, address, uintptr_t(&buffer), sizeof(T));
		if (out_status)
			*out_status = status;
		return buffer;
	}
	...
	bool readT(uintptr_t process_id, uintptr_t address, void* buffer, size_t size);
}
