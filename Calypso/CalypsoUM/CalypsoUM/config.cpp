...
void Config::init()
{
    ...
    settings::misc::only_visible = utils::text::stob(read("Settings", "only-visible"));
    ...
    settings::soundesp::enable = utils::text::stob(read("SoundEsp", "enable"));
    settings::soundesp::mode = utils::text::utos(read("SoundEsp", "mode"));
    settings::soundesp::distance = std::stoi(read("SoundEsp", "distance"));
    settings::soundesp::frequency = std::stof(read("SoundEsp", "frequency"));
...
void Config::createConfigFile()
{
    ...
    if (configFile.is_open())
    {
        ...
        configFile << "only-visible: false   # check if enemy is behind the wall or not(works with delay)\n";
        ...
        configFile << "mode: CROSSHAIR  # POSITION/CROSSHAIR\n";
        configFile << "distance: 50\n";
        configFile << "frequency: 7.0   # if beep is too fast try to play with this value\n\n";
